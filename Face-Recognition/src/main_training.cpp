// This source code is for varying probability to detect faces at different distances
...
//---------------------------------------
#ifdef SHOW_LEGEND
        cv::putText(frame, cv::format("Angle : %0.1f", obj.Angle), cv::Point(10, 40), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(0, 0, 0));
        cv::putText(frame, cv::format("Face prob : %0.4f", obj.FaceProb), cv::Point(10, 60), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(0, 0, 0));
        cv::putText(frame, cv::format("Name prob : %0.4f", obj.NameProb), cv::Point(10, 80), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(0, 0, 0));
#ifdef TEST_LIVING
        if (obj.Color == 2) {
            // face is too tiny
            cv::putText(frame, cv::format("Live prob : ??"), cv::Point(10, 100), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(0, 0, 0));
        } else {
            // face is ok
            cv::putText(frame, cv::format("Live prob : %0.4f", obj.LiveProb), cv::Point(10, 100), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(0, 0, 0));
        }
#endif // TEST_LIVING
#endif // SHOW_LEGEND
                ...
                if (FaceCnt > 0) {
                  ...
                  // edited code
                     ...
                     if (Faces[i].rect.height > MinHeightFace)  {
                        ...
                        else if(Faces[i].NameProb >= 0.60){
                        	 Faces[i].Color = 2; // Found face in database although of small size
                    	...
                    	if(Faces[i].rect.height > MinHeightFace && Faces[i].NameProb < MinFaceThreshold){
                    		 Faces[i].Color = 3;
                    	}
                    	else{
                    		Faces[i].Color = -1;
