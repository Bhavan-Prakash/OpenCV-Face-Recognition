...
const int RetinaWidth      = 700; //600 original
const int RetinaHeight     = 650; //550 original
const float MinFaceThreshold = 0.55;
const int   MinHeightFace    = 50; //50 original
...
std::queue<EmployeeInfo> emp_queue;
std::mutex queueMutex;
std::condition_variable queueCondVar;

std::set<std::string> processed_codes;
...
void makeCurlRequest(const std::string& emp_code) {
    std::time_t currentTime = std::time(nullptr);
    if (lastRequestTime.find(emp_code) != lastRequestTime.end()) {
        std::time_t lastReq = lastRequestTime[emp_code];
        if (currentTime - lastReq <= 30 * 60) {
            std::cerr << "Curl request already made for employee code "
                      << emp_code << " within 30 minutes. Skipping.\n";
            return;
        }
    }
    std::string apiKey = "qwerty!";
    std::string checkout = "Check-in";
    std::string postData = "apikey=" + apiKey + "&atype=" + checkout + "&emp_code=" + emp_code;
    std::string curlCommand = "curl -X POST -d \"" + postData + "\" attendance_portal link";

    FILE* pipe = popen(curlCommand.c_str(), "r");
    if (!pipe) {
        std::cerr << "Error: popen failed for " << emp_code << "\n";
    ...
    if (curlResult != 0) {
        std::cerr << "Error: Curl request failed for employee code " << emp_code << "\n";
    } else if (result.find("Success") != std::string::npos) {
        std::cerr << "Success: POST request using cURL for " << emp_code << "\n";
        lastRequestTime[emp_code] = currentTime;
        curlRequestCount[emp_code]++;
        std::cerr << "Total curl requests for " << emp_code << ": "
                  << curlRequestCount[emp_code] << "\n";
    } else {
        std::cerr << "Server error for employee code " << emp_code << "\n";
    }
}
...
void consumerThread() {
    while (true) {
        EmployeeInfo info;
        {
            ...
            queueCondVar.wait(lock, [] { return !emp_queue.empty(); });
            info = emp_queue.front();
            emp_queue.pop();
        }
        if (processed_codes.find(info.emp_code) == processed_codes.end()) {
            makeCurlRequest(info.emp_code);
            processed_codes.insert(info.emp_code);
        }
        while (!failedRequests.empty()) {
            std::string failed = failedRequests.front();
            failedRequests.pop();
            std::unique_lock<std::mutex> lock(queueMutex);
            emp_queue.push({failed, std::time(nullptr)});
            queueCondVar.notify_one();
        }
    }
}

// void drawLabeledBoxes(cv::Mat &frame, const std::vector<FaceObject>& Faces, const std::vector<cv::String>& NameFaces, float ScaleX, float ScaleY) {
//     for (size_t i = 0; i < Faces.size(); i++) {
//         const FaceObject &obj = Faces[i];
//         // Scale the face rectangle to the original frame size.
//         cv::Rect scaledRect;
//         scaledRect.x = static_cast<int>(obj.rect.x * ScaleX);
//         scaledRect.y = static_cast<int>(obj.rect.y * ScaleY);
//         scaledRect.width = static_cast<int>(obj.rect.width * ScaleX);
//         scaledRect.height = static_cast<int>(obj.rect.height * ScaleY);
//         cv::rectangle(frame, scaledRect, cv::Scalar(0, 255, 0), 2);

//         std::string label;
//         if (obj.NameIndex >= 0 && obj.NameIndex < NameFaces.size()) {
//             label = NameFaces[obj.NameIndex];
//         } else {
//             label = "Stranger";
//         }
...
//         int labelX = scaledRect.x;
//         int labelY = std::max(scaledRect.y - labelSize.height - baseLine, 0);
//         cv::rectangle(frame, cv::Rect(labelX, labelY, labelSize.width, labelSize.height + baseLine), cv::Scalar(0, 255, 0), cv::FILLED);
//         cv::putText(frame, label, cv::Point(labelX, labelY + labelSize.height), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(0, 0, 0), 1);
//         #ifdef SHOW_LEGEND
//         cv::putText(frame, cv::format("\n\nAngle : %0.1f \n", obj.Angle),cv::Point(10,40),cv::FONT_HERSHEY_SIMPLEX,0.6, cv::Scalar(180, 180, 0));
//         cv::putText(frame, cv::format("Face prob : %0.4f\n\n", obj.FaceProb),cv::Point(10,60),cv::FONT_HERSHEY_SIMPLEX,0.6, cv::Scalar(180, 180, 0));
//         cv::putText(frame, cv::format("Name prob : %0.4f\n\n", obj.NameProb),cv::Point(10,80),cv::FONT_HERSHEY_SIMPLEX,0.6, cv::Scalar(180, 180, 0));
//         #ifdef TEST_LIVING
//                 if(obj.Color==2){
//                     //face is too tiny
...
void drawLabeledBoxes(cv::Mat &frame, const std::vector<FaceObject>& Faces, const std::vector<cv::String>& NameFaces, float ScaleX, float ScaleY) {
    for (size_t i = 0; i < Faces.size(); i++) {
        ...
        #ifdef SHOW_LEGEND
        // Position the statistics below the face rectangle
        int statsX = scaledRect.x;
        int statsY = scaledRect.y + scaledRect.height + 20;

        cv::putText(frame, cv::format("Angle: %0.1f", obj.Angle),
                   cv::Point(statsX, statsY),
                   cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(180, 180, 0));

        cv::putText(frame, cv::format("Face prob: %0.4f", obj.FaceProb),
                   cv::Point(statsX, statsY + 20),
                   cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(180, 180, 0));

        cv::putText(frame, cv::format("Name prob: %0.4f", obj.NameProb),
                   cv::Point(statsX, statsY + 40),
                   cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(180, 180, 0));

        #ifdef TEST_LIVING
        if(obj.Color == 2) {
            // face is too tiny
            cv::putText(frame, cv::format("Live prob: ??"),
                       cv::Point(statsX, statsY + 60),
                       cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(180, 180, 0));
        } else {
            // face is ok
            cv::putText(frame, cv::format("Live prob: %0.4f", obj.LiveProb),
                       cv::Point(statsX, statsY + 60),
                       cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(180, 180, 0));
        }
        #endif // TEST_LIVING
...
void producerThread() {
   ...
   // cv::VideoCapture cap("video_link");
     cv::VideoCapture cap("camera_ip");   //Movie
    if (!cap.isOpened()) {
    ...
    std::string pattern_jpg = "./img/*.jpg";
    cv::glob(pattern_jpg, NameFaces);
    size_t FaceCnt = NameFaces.size();
    if (FaceCnt > 0) {
        for (size_t i = 0; i < FaceCnt; i++) {
            cv::Mat faceImg = cv::imread(NameFaces[i]);
            fc1.push_back(ArcFace.GetFeature(faceImg));
            size_t n = NameFaces[i].rfind('/');
            if (n != std::string::npos) {
                NameFaces[i] = NameFaces[i].substr(n+1);
            }
            n = NameFaces[i].find('.');
            if(n != std::string::npos) {
                NameFaces[i] = NameFaces[i].substr(0, n);
            }
        }
    } else {
    ...
    cv::Mat frame, result_cnn;
    std::vector<FaceObject> Faces;
    while (true) {
        cap >> frame;
        if (frame.empty()) break;
        float ScaleX = static_cast<float>(frame.cols) / RetinaWidth;
        float ScaleY = static_cast<float>(frame.rows) / RetinaHeight;
        cv::resize(frame, result_cnn, cv::Size(RetinaWidth, RetinaHeight), cv::INTER_LINEAR);
        ...
        for (size_t i = 0; i < Faces.size(); i++) {
            if (Faces[i].FaceProb > MinFaceThreshold) {
                ...
                if (FaceCnt > 0) {
                    ...
                    int Pmax = std::max_element(score_.begin(), score_.end()) - score_.begin();
                    ...
                    if (Faces[i].NameProb >= MinFaceThreshold) {
                        if (Faces[i].rect.height < MinHeightFace) {
                            ...
                            size_t pos = emp_code.find('#');
                            if (pos != std::string::npos) {
                                emp_code = emp_code.substr(0, pos);
                            }

                            if (Faces[i].NameProb >= 0.60) {
                ...
                if (Faces[i].NameIndex == -1 && Faces[i].rect.height < MinHeightFace) {
                    Faces[i].Color = 2;
                }
            }
        }
        ...
        drawLabeledBoxes(frame, Faces, NameFaces, ScaleX, ScaleY);
...
//old logic without producer-consumer
...
//     const int TIME_FRAME = 60 * 30; // Time frame in seconds (30 minutes)  alreat 60*30
//     std::time_t currentTime = std::time(nullptr);
...
// //---------------------------------------
// #ifdef SHOW_LEGEND
//         cv::putText(frame, cv::format("Angle : %0.1f", obj.Angle),cv::Point(10,40),cv::FONT_HERSHEY_SIMPLEX,0.6, cv::Scalar(255, 255, 255));
//         cv::putText(frame, cv::format("Face prob : %0.4f", obj.FaceProb),cv::Point(10,60),cv::FONT_HERSHEY_SIMPLEX,0.6, cv::Scalar(255, 255, 255));
//         cv::putText(frame, cv::format("Name prob : %0.4f", obj.NameProb),cv::Point(10,80),cv::FONT_HERSHEY_SIMPLEX,0.6, cv::Scalar(255, 255, 255));
// #ifdef TEST_LIVING
//         if(obj.Color==2){
//             //face is too tiny
//             cv::putText(frame, cv::format("Live prob : ??"),cv::Point(10,100),cv::FONT_HERSHEY_SIMPLEX,0.6, cv::Scalar(180, 180, 0));
...
//      //cv::VideoCapture cap("check.mp4"); // RaspiCam
//      //cv::VideoCapture cap(0); // Assuming USB camera is the first device (index 0)
//     cv::VideoCapture cap("rtsp://admin:miet@2024@10.253.0.75"); // Movie
//     if (!cap.isOpened()) {
